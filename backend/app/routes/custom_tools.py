...
def _serialize(row: dict) -> dict:
    return {
        k: (str(row[k]) if k == "id" else row[k])
        for k in (
            "id",
            "name",
            "description",
...
def list_tools(user: dict = Depends(require("templates", "view"))):
    ...
    with get_connection() as conn:
        rows = conn.execute(
            "SELECT * FROM custom_tools WHERE tenant_id=%s ORDER BY name", (user["tenant_id"],)
        ).fetchall()
    ...
    with get_connection() as conn:
        ...
        row = conn.execute(
            """INSERT INTO custom_tools (
                   ...
                   secrets_encrypted, timeout_seconds, enabled
               ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s) RETURNING *""",
            (
                user["tenant_id"],
                payload.name,
                payload.description,
                Json(payload.input_schema),
                payload.python_code,
                encrypt(json.dumps(payload.secrets)) if payload.secrets else None,
...
def update_tool(tool_id: str, payload: ToolIn, user: dict = Depends(require("templates", "edit"))):
    ...
    with get_connection() as conn:
        ...
        row = conn.execute(
            """UPDATE custom_tools
                  SET name=%s, description=%s, input_schema=%s, python_code=%s,
                      secrets_encrypted=%s, timeout_seconds=%s, enabled=%s,
                      updated_at=now()
